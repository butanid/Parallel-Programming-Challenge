...
//	fprintf( stderr, "OpenMP is supported -- version = %d\n", _OPENMP );
	...
	for( int n = 0;  n < MAXITERATIONS; n++ )
	{
		...
		// Going through the list of cities
		for( int i = 0; i < NUMCITIES; i++ )
		{
			int capitalnumber = -1;
			float mindistance = 1.e+37;

			// Going through list of capitals
			for( int k = 0; k < NUMCAPITALS; k++ )
			{
				float dist = Distance( i, k );
				// Figuring out for city (i) which capital (k) is the city closest to
				if( dist < mindistance )
				{
					mindistance = dist;
					capitalnumber = k;
	...
	for( int k = 0; k < NUMCAPITALS; k++ )
	{
		...
		for( int i = 0; i < NUMCITIES; i++ )
		{
			// Find distance between i & k
			float dist = Distance( i, k);
			// Figuring out what actual city is closest to each capital
		...
		Capitals[k].name = Cities[closest_city].name;
	}
